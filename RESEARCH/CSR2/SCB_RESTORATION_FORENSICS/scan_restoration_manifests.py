#!/usr/bin/env python3
# KOSD forensic scanner trigger revision 2.
...
TARGETS = ["Ferrari_250GTOClassic_1962_RestorationManifest"]
...
def find_14800(raw: bytes):
    hits = []
    for needle, label in ((struct.pack("<i", TARGET_VALUE), "int32_le"),
                          (struct.pack(">i", TARGET_VALUE), "int32_be")):
        start = 0
        while True:
...
def recursive_value_hits(value, path=""):
    ...
    elif not isinstance(value, bool) and isinstance(value, (int, float)) and value == TARGET_VALUE:
        hits.append(path)
    return hits
...
def scan_bundle(bundle: Path, target_terms):
    import UnityPy
    result = {"bundle": str(bundle), "size": bundle.stat().st_size,
              "matches": [], "restoration_candidates": [], "errors": []}
    try:
    ...
    for obj in objects:
        ...
        if not any(term in joined for term in target_terms):
            continue
        entry = {"path_id": getattr(obj, "path_id", None), "type": typ,
                 "names": names,
                 "serialized_type_has_nodes": bool(getattr(getattr(obj, "serialized_type", None), "nodes", None))}
        try:
            raw = obj.get_raw_data()
            entry["raw_size"] = len(raw)
            entry["raw_14800_hits"] = find_14800(raw)
        except Exception as e:
...
def main():
    ...
    report = {"tool": "KOSD CSR2 RestorationManifest forensic scanner",
              "python": sys.version, "targets": TARGETS,
              "target_terms": TARGET_TERMS, "target_value": TARGET_VALUE,
              "obbs": [], "bundles_scanned": 0, "bundle_results": [], "summary": {}}

    with tempfile.TemporaryDirectory(prefix="kosd_scb_forensics_") as td:
        temp = Path(td)
        for obb_arg in args.obb:
            obb = Path(obb_arg)
            info = {"path": str(obb), "size": obb.stat().st_size if obb.exists() else None,
    ...
    report["summary"] = {
        ...
        "target_value_found_in_decoded_typetree": any(x["match"].get("typetree_14800_paths") for x in value_hits),
    }
